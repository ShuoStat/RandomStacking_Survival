...
generaClinMol <- function(n.obs = 200, tot.genes = 10000, n.groups = 100,
    ...
    }
    {
        if (is.null(n.gene)) {
            ...
            warning(paste0(
                "The length of n.gene is smaller then ", n.groups,
    ...
    # if we generate more genes than those indicated in tot.gene, tell how many genes are actually generated
    {
        if (sumBs > tot.genes) {
            ...
            n.gene[n.groups + 1] <- tot.genes - sumBs
        }
    }
    ...
    if (length.n.clin > n.groups) {
        ...
        warnings(paste0(
            "Number of clinical groups too large, only the first ",
...
simData <- function(n.obs,
                    t.gene,
                    r,
                    sd_sett,
                    weib_shape = 1,
                    weib_scale = 1,
                    censoring_target = 0.4) {
    # 1, generate data
    n.clin <- c(3, 3, 2, 2, 5)
    n.gene <- rep(10, 49)
    ...
    d <- generaClinMol(
        n.obs = n.obs, tot.genes = t.gene, n.groups = 49,
    ...
    # 2, generate the response variable
    X <- cbind(d$clin, d$gene)

    beta.clin <- c(3, -3, 0, 3, 0, 0, -3, 0, 0, 0, 3, 0, 0, 0, 0)
    beta.gene <- c(
        3, -3, 1, -1, 1, rep(0, 5), 2, -2, 2, rep(0, 7),
        rep(0, 10), -3, -2, -1, rep(0, 7),
        rep(-1, 5), rep(0, 5), -2, rep(0, 9),
        rep(1, 11), rep(0, (10000 - 71))
    )

    beta <- c(beta.clin, beta.gene[1:t.gene])

    # Generate survival times
    eta <- X %*% beta + rnorm(n.obs, sd = sd_sett)

    # to avoid huge values of eta
    eta <- scale(eta) * 0.5
    eta <- as.numeric(eta)

    U <- runif(n.obs)
    T_true <- (-log(U) / (weib_scale * exp(eta)))^(1 / weib_shape)

    get_censor_rate <- function(rate) {
        C <- rexp(n.obs, rate = rate)
        mean(T_true > C)
    }

    censor_rate <- uniroot(function(rate) get_censor_rate(rate) - censoring_target, lower = 1e-4, upper = 100)$root
    C <- rexp(n.obs, rate = censor_rate)
    time <- pmin(T_true, C)
    ...
    return(list(X = X, time = time, status = status, blocks = blocks))
}

# snr to sd_sett
getsd_sett <- function(snr, r = 0.5) {
    data <- simData(
        n.obs = 10000,
        t.gene = 1000,
        r = r,
    ...
    beta.gene <- c(
        ...
        rep(0, 10), -3, -2, -1, rep(0, 7),
        rep(-1, 5), rep(0, 5), -2, rep(0, 9),
        rep(1, 11), rep(0, (1000 - 71))
    )
    beta <- c(beta.clin, beta.gene)
    X <- data$X
    eta <- X %*% beta

    sd_sett <- sqrt(var(eta) / snr)
    return(sd_sett)
}
...
for (snr in snrs) {
    for (r in rs) {
        ...
        sd_setts <- rbind(sd_setts, c(snr = snr, r = r, sd_sett = sd_sett))
    }
}
...
for (n in n.obss) {
    for (snr in snrs) {
        for (r in rs) {
            ...
            sd_setts <- as.data.frame(sd_setts)
            sd_sett <- sd_setts[sd_setts$r == r & sd_setts$snr == snr, ]$sd_sett
            ...
            foreach(
                ...
                times <- quantile(data$time[data$status == 1], probs = seq(0.1, 0.9, length = 10))
