...
cumhz <- function(lp, y, times) {
  ...
  if(any(times>max(stoptimes)))
    cat("Prediction time exceeds max follow-up\n")
  # check end ---
...
survP <- function(beta, trainX, trainY, validX, times) {
  ...
  # check begin ----
  if (anyNA(chz)){
    cat("NA chz       :", anyNA(chz), "\n")
  }
  
  if (any(is.infinite(chz))) {
    cat("Inf chz      :", any(is.infinite(chz)), "\n")
  }
  
  if(any(is.infinite(exp(lpFitted)))){
    cat("max(lp):", max(lpFitted), "\n")
  }
  
  # check end -----
  
  survProb <-  mapply(function(x) {
    exp(-x * exp(lpPred))
  }, x = chz, SIMPLIFY = T)
  
  if (anyNA(survProb)) {
    cat("NA survProb  :", anyNA(survProb), "\n")
  }
...
brierScore <- function(Xclin.t, Xclin.v, y.t, y.v,
  ...
  if (nrow(out) == 0) {
    cat("Empty brierScore at time =", time, "\n")
  }
  # check end
...
optimIPS <- function(X, y, weights = rep(1, nrow(X))) {
  ...
  m <- ncol(X)
  stk <- Rsolnp::solnp(rep(1 / m, m),
    fun = optfun, eqfun = sm1, eqB = 1,
    LB = rep(0, m),
    UB = rep(1, m),
...
StkCoxSL <- function(obj,
  ...
  if (is.null(foldid)) {
    foldid <- sample(
      rep(1:nfold, length = nrow(obj$CVpred)),
      nrow(obj$CVpred)
    )
  }
  ...
  if (is.null(foldid.cox)) {
    foldid.cox <- sample(
      rep(1:nfold, length = nrow(obj$CVpredCox)),
      nrow(obj$CVpredCox)
    )
  }
