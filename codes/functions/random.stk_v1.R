...
random.StkCoxBL <- function(X, y, 
  ...
  for(i in seq_along(bootID)) {  
    ...
    if (i <= fastTune) {
      ...
      maxLam <- rbind(maxLam, unlist(lapply(mods, function(x) max(x[["lambdaSeq"]]))))
...
random.predBLBin <- function(obj, newdata, type.BL = "response") {
  predBLBin(obj, newdata, type.BL = "response", ifCVMods = TRUE)
}

random.predSLBin <- function(BLBin, alphas, type = c("link", "response")) {
  predSLBin(BLBin = BLBin, alphas = alphas, type = type)
}
