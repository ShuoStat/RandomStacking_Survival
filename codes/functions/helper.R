...
getTimeIBS <- function(survProb, 
    ...
    for(i in seq_along(times)){
        bs <- brierScore(Xclin.t, Xclin.v, y.t, y.v, 
                         survProb[,i], times[i], cens.method = cens.method)
...
getTimeAUC <- function(survProb, Xclin.t, Xclin.v, y.t, y.v, 
    ...
    aucs <- matrix(NA, nrow = length(times), ncol = 2, 
                   dimnames = list(seq_along(times),
                                   c("time", "auc")))
    
    for(i in seq_along(times)){
        
        bs <- brierScore(Xclin.t, Xclin.v, 
                         y.t, y.v, 
                         survProb[,i], times[i], 
...
predProbs <- function(X.v, obj, times, ifCVMods = FALSE){
  ...
  for(i in stkMod) {
    if (grepl("^Rstk", i)) {
      re[[i]] <- predict.random.stack.cox(obj[[i]], X.v, times = times)
    } else {
      re[[i]] <- predict.stack.cox(obj[[i]], X.v, times = times, ifCVMods = ifCVMods)
    }
  }
